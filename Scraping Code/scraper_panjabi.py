...
# Configuration
SEARCH_URL = "https://www.aarong.com/bgd/catalogsearch/result?q=Panjabi&product_list_order=high_to_low"
OUTPUT_DIR = os.path.join(os.getcwd(), "New punjabi for man")
TARGET_COUNT = 1500
...
def scrape_product(driver, url):
    ...
    # Method 2: Standard img tags (Fallback/Additional)
    # Always check for standard images to ensure we get everything
    if True:
        ...
        for img in imgs:
            ...
            if src and 'media/catalog/product' in src:
                ...
                if src.startswith('http'):
                    ...
                    # Usually related products have 1 image each. If we get 3+ unique images, it's likely the main gallery.
                    image_urls.add(src)
    
    if len(image_urls) == 0:
        print(f"Skipping: Found 0 images")
        return False
...
def main():
    ...
    try:
        ...
        # Save debug HTML
        with open("debug_panjabi.html", "w", encoding="utf-8") as f:
            f.write(driver.page_source)
        print("Saved debug_panjabi.html")
